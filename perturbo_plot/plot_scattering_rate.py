...
def main() -> None:
    ...
    if args.unit == "ps^-1":
        ax.set_ylabel(r"Scattering Rate ($\mathbf{ps}^{\mathbf{-1}}$)" if want_bold else r"Scattering Rate (ps$^{-1}$)")
    else:
        ax.set_ylabel(r"$\mathbf{1}/\boldsymbol{\tau}$ ($\mathbf{s}^{\mathbf{-1}}$)" if want_bold else r"$1/\tau$ (s$^{-1}$)")
